...
function SalesOrders() {
  ...
  useEffect(() => {
    async function fetchRequests() {
      ...
      let query = supabase
        ...
        .select('*, customer:customer_id(id, name, company_code), company:company_id(name), status:status_id!inner(name)')
  ...
  useEffect(() => {
    async function fetchIndicators() {
      ...
      let query = supabase
        .from('vw_credit_limit_dashboard')
        .select('*')
        .eq('company_id', companyId);
        
      if (filterBranch) {
        query = query.eq('company_id', filterBranch);
      }
      const { data, error } = await query;
      if (!error && data) {
